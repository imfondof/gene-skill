...
一个用于生成可被大模型直接调用的人格技能 `SKILL.md` 的方法论仓库。

它把一个人物或概念视为可被"测序"的系统，围绕六条维度、双链提取协议和情境调度层，生成一份既可阅读、也可被 AI 直接加载运行的技能文件。
...
1. `Clone`
从真实人物出发，基于公开资料或用户提供素材，构建其人格技能。

2. `Evolve`
在已有 `SKILL.md` 的基础上，结合新信息做增量更新。

3. `Create`
从多个技能、概念描述或两者混合中，创造一个新的人格实体。
...
- `维度1·认知`：心智模型、决策启发式、信念强度
- `维度2·情绪`：基线情绪、触发器、调节策略、应激模式
- `维度3·关系`：角色映射、权力偏好、信任与边界
- `维度4·叙事`：核心神话、语言指纹、自我叙事
- `维度5·时间`：跃迁事件、进化轨迹、当前向量、预测方向
- `维度6·阴影`：盲区、防御机制、未表达潜能
...
模板位于 [references/skill-template.md](references/skill-template.md)。
...
4. 最后看示例 [examples/qin-shi-huang-skill/SKILL.md](examples/qin-shi-huang-skill/SKILL.md)
这是目前最直观的完整输出样例。
...
- [examples/qin-shi-huang-skill/SKILL.md](examples/qin-shi-huang-skill/SKILL.md) — 秦始皇（历史人物，Clone 路径）
- [examples/linzhizhan-skill/SKILL.md](examples/linzhizhan-skill/SKILL.md) — 林知栈（虚构算法教员，Create 路径）
